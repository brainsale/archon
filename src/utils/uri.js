...
export const getHttpUri = (uri, ipfsGateway) => {
  ...
  switch (protocol) {
    case "http":
      break;
    case "https": {
      if (ipfsGateway === "https://ipfs.kleros.io" || ipfsGateway === "https://ipfs.fleek.co") {
        preValidated = true;
      }
      if (uri.includes("kleros.io/ipfs")) {
        uri = uri.replace("kleros.io", "fleek.co");
      }
    ...
    case "ipfs":
      ...
      else if (uri.substr(0, 6) === "ipfs:/") uri = `${ipfsGateway}/${uri.split(":/").pop()}`;
      else throw new Error(`Unrecognized protocol ${protocol}`);
